-- Finale Lösung mit expliziter Transaktion und DEFERRED Constraints
...
BEGIN
  ...
  BEGIN
    ...
    LOOP
      ...
      IF (temp_q_id IS NOT NULL OR temp_o_id IS NOT NULL) AND i <= array_length(question_ids, 1) THEN
        -- Logge die Abhängigkeiten für Debugging
        RAISE NOTICE 'Setze Abhängigkeit für Frage %: depends_on_question_id = %, depends_on_option_id = %', 
                      question_ids[i], temp_q_id, temp_o_id;
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    RAISE NOTICE 'Fehler in der Transaktion: %', SQLERRM;
    result := jsonb_build_object(
      'success', false,
      'error', SQLERRM
    );
