...
export const BiopsyFormData = ({
	...
	const {
		...
		queryKey: ["biopsy_form_submissions", appointment.id, refreshKey],
		queryFn: async () => {
        ...
        `
			...
			try {
				return BiopsyFormSchema.parse(data) as BiopsyFormType;
			} catch (valError) {
	...
	const saveMutation = useMutation({
		...
		onSuccess: async () => {
			...
			queryClient.refetchQueries({
				queryKey: ["biopsy_form_submissions", appointment.id],
			});
	...
	useEffect(() => {
		if (!isLoadingSubmission) {
			setData({
				...data,
				submission: submission as BiopsyFormType,
