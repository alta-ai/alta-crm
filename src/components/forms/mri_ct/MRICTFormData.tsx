...
import {
	type MRICTForm as MRICTFormType,
	type Appointment,
	type Patient,
	MRICTFormSchema,
} from "../../types";
...
export const MRICTFormData = ({
	...
	const {
		...
		queryKey: ["mri_ct_form_submissions", appointment.id, refreshKey],
		queryFn: async () => {
			const { data, error } = await supabase
				.from("mri_ct_form_submissions")
				.select(
					`
          *
        `
			...
			try {
				return MRICTFormSchema.parse(data) as MRICTFormType;
			} catch (valError) {
	...
	const saveMutation = useMutation({
		...
		onSuccess: async () => {
			...
			queryClient.refetchQueries({
				queryKey: ["mri_ct_form_submissions", appointment.id],
			});
	...
	useEffect(() => {
		if (!isLoadingSubmission) {
			setData({
				...data,
				submission: submission as MRICTFormType,
