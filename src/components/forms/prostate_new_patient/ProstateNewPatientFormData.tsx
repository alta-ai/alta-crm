...
export interface ProstateNewPatientDataContextType {
	submission: ProstateNewPatientFormType;
	patient: Patient;
}
...
export const ProstateNewPatientFormData = ({
	...
	const {
		...
		queryFn: async () => {
			const { data, error } = await supabase
				.from("prostate_new_patient_form_submissions")
				.select(
					`
          *
        `
			...
			try {
				return ProstateNewPatientFormSchema.parse(
					data
				) as ProstateNewPatientFormType;
			} catch (valError) {
	...
	const saveMutation = useMutation({
		...
		onSuccess: async () => {
			...
			queryClient.refetchQueries({
				queryKey: ["prostate_new_patient_form_submissions", appointment.id],
			});
	...
	useEffect(() => {
		if (!isLoadingSubmission) {
			setData({
				...data,
				submission: submission as ProstateNewPatientFormType,
...
export default ProstateNewPatientFormData;
