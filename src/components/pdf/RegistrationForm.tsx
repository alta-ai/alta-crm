...
export const RegistrationForm = () => {
  const { formData, patientData, appointmentData } = useFormData();
      ...
      <BaseFormPage>
        ...
        <View style={styling.FormTypeLines}>
          ...
          <FormRow
            ...
            items={[
              {
                label: 'PKV/GKV',
                value: patientData?.insurance?.privateInsurance || patientData?.insurance?.publicInsurance,
                start: 50,
                type: 'text',
              },
              {
                ...
                value: (
                  <BoxSelection
                    options={['Ja', 'Nein']}
                    selected={booleanToYesNo(patientData?.insurance?.eligibleForAid)}
        ...
        <View style={styles.QuestionBox}>
          ...
          >
            <BoxSelection
              options={['Ja', 'Nein']}
              selected={booleanToYesNo(formData?.urologicInformation?.currentlyUndergoingTreatment)}
          ...
          >
            <BoxSelection
              options={['Ja', 'Nein']}
              selected={booleanToYesNo(formData?.urologicInformation?.visitDueToRecommendation)}
              style={styles.QuestionBox.BoxSelection}
            />
          </Question>
          <Question
          ...
          >
            <BoxSelection
              options={['Ja', 'Nein']}
              selected={booleanToYesNo(formData?.urologicInformation?.gotTransfer)}
              style={styles.QuestionBox.BoxSelection}
            />
          </Question>
          <Question question="Soll der Befundbericht an den Behandler geschickt werden?">
            <BoxSelection
              options={['Ja', 'Nein']}
              selected={booleanToYesNo(formData?.urologicInformation?.sendReportToUrologist)}
