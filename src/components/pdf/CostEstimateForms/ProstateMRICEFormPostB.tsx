...
interface ProstateMRICEFormPostBProps {
	onRender?: () => void;
}
...
export const ProstateMRICEFormPostB: React.FC<ProstateMRICEFormPostBProps> = (
	props
) => {
	const {
		appointmentData,
		...
		<Document style={styles.Page} onRender={props?.onRender}>
			<BaseFormPage customStyle={{ fontSize: "10px" }}>
				<Header>
					...
					<Text style={styles.SubHeading}>
						der Prostata und des gesamten Beckens (PostB)
					</Text>
				</Header>
