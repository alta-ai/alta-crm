...
const ExaminationForm = () => {
	...
	const [formBillingTypes, setFormBillingTypes] = useState<
	...
	const {
		fields: billingCodeFields,
		append: appendBillingCode,
		remove: removeBillingCode,
	} = useFieldArray({
		control,
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				const { data: billingCodesData, error: billingCodesError } =
					await supabase
						.from("billing_codes")
            ...
            `
					...
					if (examination) {
						...
						const billingCodeIds = examination.examination_billing_codes
							? examination.examination_billing_codes.map(
									(ebc: any) => ebc.billing_code_id
							  )
							: [];

						reset({
							...examination,
							devices: examination.examination_devices.map(
								(ed: any) => ed.device_id
							),
	...
	const onSubmit = async (data: ExaminationFormData) => {
		...
		try {
			if (id) {
				...
				if (data.forms && data.forms.length > 0) {
					const formAssociations = data.forms.map((form, index) => ({
						...
						order: index + 1,
						billing_type:
							formBillingTypes[form.id] || BILLING_TYPES.map((bt) => bt.value),
					}));
				...
				if (data.billing_codes && data.billing_codes.length > 0) {
					const billingCodeAssociations = data.billing_codes.map(
						(billingCode) => ({
							examination_id: id,
							billing_code_id:
								typeof billingCode === "string" ? billingCode : billingCode.id,
						})
					);
				...
				if (examination) {
					await supabase.from("examination_devices").insert(
					...
					await supabase.from("examination_forms").insert(
						data.forms.map((form, index) => ({
							examination_id: examination.id,
							form_id: form.id,
							order: index + 1,
							billing_type:
								formBillingTypes[form.id] ||
								BILLING_TYPES.map((bt) => bt.value),
						}))
					);

					await supabase.from("examination_sequences").insert(
						data.sequences_without_contrast.map((seq) => ({
							examination_id: examination.id,
							name: seq.name,
							with_contrast: false,
					...
					await supabase.from("examination_billing_codes").insert(
						data.billing_codes.map((billingCode) => ({
							examination_id: examination.id,
							billing_code_id:
								typeof billingCode === "string" ? billingCode : billingCode.id,
						}))
			...
			setError(
				error.message || "Failed to save examination. Please try again later."
			);
		} finally {
			setLoading(false);
		}
	...
	const toggleBillingType = (formId: string, billingType: string) => {
		const currentTypes =
			formBillingTypes[formId] || BILLING_TYPES.map((bt) => bt.value);
		let newTypes: string[];
					...
					<div className="grid grid-cols-1 gap-6">
							...
							<div className="mt-2">
								<label className="inline-flex items-center">
									...
									<span className="ml-2 text-sm text-gray-700">
										Körperseite (links/rechts) muss bei der Terminbuchung
										angegeben werden
									</span>
								</label>
							</div>
						...
						<div>
							...
							<input
								...
								placeholder={
									requiresBodySide
										? "z.B. MRT Kopf {{bodyside}}"
										: "z.B. MRT Kopf"
								}
							/>
							<p className="mt-1 text-sm text-gray-500">
								Verfügbare Platzhalter: {"{{bodyside}}"} für die Körperseite,{" "}
								{"{{date}}"} für das Datum
							</p>
						</div>
							...
							<div className="space-y-3">
								{billingCodeFields.map((field, index) => (
									<div key={field.id} className="flex items-center space-x-3">
										<select
											{...register(`billing_codes.${index}.id` as const, {
												required: "Abrechnungsziffer ist erforderlich",
											})}
								...
								{billingCodeFields.length === 0 && (
									<div className="text-sm text-gray-500 italic py-2">
										Noch keine Abrechnungsziffern hinzugefügt. Klicken Sie auf
										"Weitere Ziffer hinzufügen" um Abrechnungsziffern zu dieser
										Untersuchung hinzuzufügen.
									</div>
								)}
							</div>
						</div>
								...
								{devices.map((device) => (
									<label key={device.id} className="flex items-center">
										<input
											...
											value={device.id}
											{...register("devices", {
												required: "Mindestens ein Gerät muss ausgewählt werden",
											})}
											className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
										/>
										<span className="ml-2 text-sm text-gray-700">
										...
										{sequencesWithoutContrast.map((sequence, index) => (
											...
											>
												<input
													type="text"
													{...register(
														`sequences_without_contrast.${index}.name` as const
													)}
												...
												<label className="flex items-center">
													<input
														type="checkbox"
														{...register(
															`sequences_without_contrast.${index}.is_standard` as const
														)}
														className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
													/>
													<span className="ml-2 text-sm text-gray-700">
										...
										{sequencesWithContrast.map((sequence, index) => (
											...
											>
												<input
													type="text"
													{...register(
														`sequences_with_contrast.${index}.name` as const
													)}
													placeholder="Sequenzname"
													className="flex-1 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 text-sm"
												/>
												<label className="flex items-center">
													<input
														type="checkbox"
														{...register(
															`sequences_with_contrast.${index}.is_standard` as const
														)}
														className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
													/>
													<span className="ml-2 text-sm text-gray-700">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
							<div>
								...
								{errors.self_payer_without_contrast && (
									<p className="mt-1 text-sm text-red-600">
										{errors.self_payer_without_contrast.message}
									</p>
								)}
							</div>
							...
							<div>
								...
								{errors.self_payer_with_contrast && (
									<p className="mt-1 text-sm text-red-600">
										{errors.self_payer_with_contrast.message}
									</p>
								)}
							</div>
							...
							<div>
								...
								{errors.private_patient_without_contrast && (
									<p className="mt-1 text-sm text-red-600">
										{errors.private_patient_without_contrast.message}
									</p>
								)}
							</div>
							...
							<div>
								...
								{errors.private_patient_with_contrast && (
									<p className="mt-1 text-sm text-red-600">
										{errors.private_patient_with_contrast.message}
									</p>
								)}
							</div>
							...
							<div>
								...
								{errors.foreign_patient_without_contrast && (
									<p className="mt-1 text-sm text-red-600">
										{errors.foreign_patient_without_contrast.message}
									</p>
								)}
							</div>
							...
							<div>
								...
								{errors.foreign_patient_with_contrast && (
									<p className="mt-1 text-sm text-red-600">
										{errors.foreign_patient_with_contrast.message}
									</p>
								)}
							</div>
						</div>
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								<div className="border rounded-lg p-4">
									...
									<div className="space-y-2">
										{allForms
											.filter(
												(form) => !watchedForms.some((wf) => wf.id === form.id)
											)
											.map((form) => (
														...
														onChange={(e) => {
															...
															if (e.target.checked) {
																setValue("forms", [
																	...
																	{
																		id: form.id,
																		billing_types: BILLING_TYPES.map(
																			(bt) => bt.value
																		),
																	},
																]);
									...
									<div className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-4">
										<div className="flex">
											...
											<p className="text-sm text-blue-700">
												Wählen Sie für jedes Formular die Abrechnungsarten aus,
												bei denen es verwendet werden soll.
											</p>
										</div>
									</div>
