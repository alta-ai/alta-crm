...
const BillingFormFilledView: React.FC<BillingFormFilledViewProps> = ({ 
  ...
  useEffect(() => {
    const loadFormData = async () => {
      try {
        ...
        console.log('Geladene Fragen mit Abhängigkeiten:', questionsData.map(q => ({
          id: q.id,
        ...
        for (const question of questionsData) {
          ...
          if (question.depends_on_question_id) {
            console.log(`Optionen für abhängige Frage ${question.question_text}:`, optionsData);
          }
        ...
        const { data: existingFormData, error: existingFormError } = await supabase
        ...
        if (existingFormData) {
          ...
          existingAnswers.forEach(answer => {
            const question = formattedQuestions.find(q => q.id === answer.question_id);
            if (!question) return;
            ...
            if (question.question_type === 'multiple_choice') {
              ...
              savedAnswers[answer.question_id] = answer.number_answer !== null ? answer.number_answer : '';
            } else if (question.question_type === 'bullet_points') {
              if (!savedAnswers[answer.question_id]) {
                savedAnswers[answer.question_id] = question.options.map(() => false);
              }
              const optionIndex = question.options.findIndex(opt => opt.id === answer.option_id);
              if (optionIndex >= 0) {
  ...
  const handleAnswerChange = (questionId: string, value: any) => {
    setAnswers(prev => {
      ...
      const directDependentQuestions = questions.filter(q => q.depends_on_question_id === questionId);
      ...
      const updateDependentQuestions = (dependsOnQuestionId: string) => {
        ...
        const dependentQuestions = questions.filter(q => q.depends_on_question_id === dependsOnQuestionId);
        
        dependentQuestions.forEach(question => {
          const shouldShow = shouldShowQuestionWithAnswers(question, updatedAnswers);
          ...
          // Wenn Frage nicht mehr sichtbar ist, entferne Antworten
            ...
            questions.filter(q => q.depends_on_question_id === question.id).forEach(q => {
  ...
  const shouldShowQuestionWithAnswers = (question: FormQuestion, currentAnswers: Record<string, any>) => {
    ...
    if (Array.isArray(dependentAnswer)) {
      ...
      console.log(`multiple_choice: ${result ? 'Zeige Frage' : 'Verstecke Frage'}`);
      return result;
    } else {
      ...
      console.log(`single_choice/yes_no: ${result ? 'Zeige Frage' : 'Verstecke Frage'}`);
      return result;
    }
  };
  ...
  const validateForm = () => {
    ...
    questions.forEach(question => {
      ...
      if (!shouldShowQuestionWithAnswers(question, answers) || !question.required) return;
      ...
      if (question.question_type === 'yes_no' || question.question_type === 'single_choice') {
        ...
        isValid = answer !== null && answer !== undefined && answer.trim() !== '';
      } else if (question.question_type === 'number') {
        isValid = answer !== null && answer !== undefined && answer !== '';
      } else if (question.question_type === 'bullet_points') {
        isValid = Array.isArray(answer) && answer.some(checked => checked === true);
      }
        ...
        {questions.map(question => (
          shouldShowQuestionWithAnswers(question, answers) && (
            <div 
              ...
              className={`border ${validationErrors[question.id] ? 'border-red-300 bg-red-50' : 'border-gray-200'} rounded-md p-4`}
            >
              ...
              <div className="mt-2">
                ...
                {question.question_type === 'single_choice' && (
                  <div className="flex flex-col space-y-2">
                    {question.options.map(option => (
                      <label key={option.id} className="flex items-start cursor-pointer py-1">
                        <input
                          type="radio"
                          id={`${question.id}-${option.id}`}
                          name={question.id}
