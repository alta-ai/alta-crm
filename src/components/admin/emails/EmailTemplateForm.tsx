...
const FORM_TYPES = [
	...
	},
	{
		...
		fields: [{ id: "fgmo", label: "Kostenerstattung bestätigt" }],
	},
	{
		id: "privacy",
...
const CONDITION_FIELDS = {
	...
	examination: [
		...
		{
			id: "examination.with_contrast_medium",
			label: "Mit Kontrastmittel",
			type: "boolean",
		},
	],
	location: [
...
const PLACEHOLDERS = {
	patient: [
		{ id: "patient.title", label: "Titel" },
		{
			...
			label: "Anrede (Sehr geehrte/r basierend auf Geschlecht)",
		},
...
const FIELD_MAPPING: Record<string, string> = {
	...
	"appointment.with_contrast_medium": "appointment.with_contrast_medium",
	"appointment.has_transfer": "appointment.has_transfer",
	"appointment.referring_doctor": "appointment.referring_doctor",
...
const EmailTemplateForm = () => {
	...
	const [visiblePlaceholders, setVisiblePlaceholders] = useState<string[]>([
		"patient",
		"examination",
		"appointment",
		"location",
	...
	useEffect(() => {
		if (selectedFormType) {
			const predefinedForm = FORM_TYPES.find(
				(form) => form.id === selectedFormType
			);
			if (predefinedForm) {
	...
	const onSubmit = async (data: EmailTemplateFormData) => {
		...
		try {
			...
			setError(
				error.message || "Failed to save template. Please try again later."
			);
		} finally {
			setLoading(false);
		}
	...
	const logObjectStructure = (
		...
		for (const key in obj) {
			if (obj.hasOwnProperty(key)) {
				...
				const value = obj[key];
				console.log(
					`${path}: ${typeof value} ${value === null ? "(null)" : ""}`
				);
	...
	const handlePreview = async () => {
		try {
			...
			const { data: latestAppointment, error: appointmentError } =
				await supabase
					.from("appointments")
					.select(
						`
        ...
        `
			...
			if (!latestAppointment)
				throw new Error("Kein Termin gefunden für die Vorschau");
			...
			setError(
				"Fehler beim Erstellen der Vorschau. Bitte versuche es später noch einmal."
			);
		} finally {
			setLoading(false);
		}
	...
	const getActualValue = (
		...
		if (debugMode) {
			console.log(
				`Versuche Wert zu holen für: ${fieldId} (gemappt zu: ${mappedFieldId})`
			);
		}
		...
		for (let i = 1; i < parts.length; i++) {
			if (!objCategory || objCategory[parts[i]] === undefined) {
				if (debugMode) {
					console.warn(
						`Feld ${parts[i]} nicht in ${parts.slice(0, i).join(".")} gefunden`
					);
				}
				return `[${getExampleValue(category, fieldId)}]`;
			}
		...
		if (fieldId === "appointment.start_time" && objCategory) {
			try {
				return format(new Date(objCategory), "dd.MM.yyyy, HH:mm 'Uhr'", {
					locale: de,
				});
			} catch (e) {
				if (debugMode) {
	...
	const getExampleValue = (category: string, fieldId: string) => {
		switch (category) {
			...
			case "examination":
				...
				break;
			case "appointment":
				if (fieldId === "appointment.start_time")
					return "15.04.2025, 14:30 Uhr";
				...
				if (fieldId === "appointment.forms_url")
					return "https://klinik.de/formulare/123";
				break;
			case "location":
				if (fieldId === "location.name") return "Radiologie Zentrum Mitte";
				if (fieldId === "location.address")
					return "Hauptstraße 1, 10115 Berlin";
				if (fieldId === "location.phone") return "030 123456";
				if (fieldId === "location.email")
					return "kontakt@radiologie-zentrum.de";
				if (fieldId === "location.directions")
					return "U-Bahn Linie U2, Haltestelle Stadtmitte";
				break;
			case "device":
	...
	const renderConditionValueField = (
		...
		for (const category in CONDITION_FIELDS) {
			const field = CONDITION_FIELDS[
				category as keyof typeof CONDITION_FIELDS
			...
			{debugMode && (
				<div className="mb-6 bg-yellow-50 border-l-4 border-yellow-500 p-4">
					<p className="text-yellow-700 text-sm">
						Debug-Modus aktiviert. Detaillierte Logs werden in der Konsole
						angezeigt.
					</p>
				</div>
			)}
								...
								<div>
									...
									<select
										{...register("trigger_type", {
											required: "Trigger-Typ ist erforderlich",
										})}
										className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200"
									>
										<option value="">Bitte wählen</option>
						...
						<div className="mt-6">
							...
							<select
								{...register("recipient_field", {
									required: "Empfänger-Feld ist erforderlich",
								})}
								className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200"
							>
								<option value="">Bitte wählen</option>
							...
							{errors.recipient_field && (
								<p className="mt-1 text-sm text-red-600">
									{errors.recipient_field.message}
								</p>
							)}
						</div>
					)}
					...
					<div className="mt-8">
						...
						<div className="bg-gray-50 p-4 rounded-lg">
							<p className="text-sm text-gray-600 mb-4">
								Definiere Bedingungen, unter denen diese E-Mail gesendet werden
								soll.
							</p>
							...
							{conditionGroups.length === 0 ? (
								<div className="text-center p-8">
									<p className="text-gray-500 mb-4">
										Keine Bedingungen definiert. Diese E-Mail wird immer
										gesendet.
									</p>
									<button
										type="button"
										...
										>
												...
												<div className="flex items-center">
													...
													{groupIndex > 0 && (
														<select
															...
															onChange={(e) =>
																updateGroupOperator(groupIndex, e.target.value)
															}
											...
											{group.conditions.length === 0 ? (
												<p className="text-gray-500 text-sm mb-2">
													Keine Bedingungen in dieser Gruppe.
												</p>
											) : (
												group.conditions.map(
													(condition: any, conditionIndex: number) => (
														<div
															key={conditionIndex}
															className="mb-3 grid grid-cols-12 gap-2 items-center"
														>
															{conditionIndex > 0 && (
															...
															>
																...
																>
																	...
																	<optgroup label="Untersuchung">
																		{CONDITION_FIELDS.examination.map(
																			(field) => (
																				<option key={field.id} value={field.id}>
																					{field.label}
																				</option>
										...
										<div>
											...
											<div className="mt-2 space-y-4">
												...
												{Object.entries(PLACEHOLDERS)
													.filter(([category]) =>
														visiblePlaceholders.includes(category)
													)
													.map(([category, fields]) => (
														<div key={category}>
					...
					<div className="bg-white rounded-lg p-6 max-w-3xl w-full max-h-[90vh] overflow-y-auto">
									...
									{previewData.appointment && (
										<div className="mt-1 text-sm text-blue-700">
											...
											<p className="mt-1">
												{selectedScheduleType === "immediate" ? (
													<span>
														Diese E-Mail wird sofort nach dem Trigger
														verschickt.
													</span>
												) : (
													<span>
														Diese E-Mail wird{" "}
														{selectedScheduleType === "before_appointment"
															? "vor"
															: "nach"}{" "}
														dem Termin verschickt:
														<strong>
															{" "}
															{formatPreviewDate(previewData.previewSendDate)}
														</strong>
													</span>
												)}
											</p>
												...
												<div className="mt-2 pt-2 border-t border-blue-200">
													{previewData.conditionsMet ? (
														<p className="flex items-center text-green-700">
															...
															Mit den aktuellen Bedingungen würde diese E-Mail
															für diesen Termin versendet werden.
														</p>
													) : (
														<p className="flex items-center text-red-700">
															<AlertTriangle className="h-4 w-4 mr-1" />
															Mit den aktuellen Bedingungen würde diese E-Mail
															für diesen Termin NICHT versendet werden.
														</p>
													)}
												</div>
											)}
						...
						<div className="mb-4">
							<div className="bg-gray-100 p-3 rounded mb-2">
								<div>
									<strong>Von:</strong>{" "}
									{watch("sender_email") || "noreply@example.com"}
								</div>
								<div>
									<strong>An:</strong>{" "}
									{previewData.patient?.email || "patient@example.com"}
								</div>
								<div>
									<strong>Betreff:</strong> {subject || "Kein Betreff"}
								</div>
							</div>
						</div>
