...
const PatientHistory = () => {
	...
	const [editingDocumentComment, setEditingDocumentComment] = useState<
		string | null
	>(null);
	...
	const [commentFilter, setCommentFilter] = useState<
		"all" | "completed" | "pending" | "info" | "anruf" | "priority"
	>("all");
	...
	const handleMarkCommentAsCompleted = async (commentId: string) => {
		try {
			...
			setError(
				err.message || "Fehler beim Markieren des Kommentars als erledigt"
			);
		}
	};
		...
		switch (commentFilter) {
			case "completed":
				return comments.filter(
					(comment) => comment.is_todo && comment.completed
				);
			case "pending":
				return comments.filter(
					(comment) => comment.is_todo && !comment.completed
		...
		<div>
			...
			<div className="bg-white shadow-sm rounded-lg overflow-hidden">
				<div className="px-6 py-4 border-b border-gray-200">
					<h2 className="text-lg font-medium text-gray-900">Untersuchungen</h2>
				</div>
								...
								<div className="flex items-start justify-between">
									<div className="flex-1">
										...
										<div className="mt-1 text-sm text-gray-500 space-y-1">
											...
											<p>
												{appointment.device?.name || "Kein Gerät"} •{" "}
												{appointment.location?.name || "Kein Standort"}
											</p>
											<p>
												Abrechnungsart:{" "}
												{appointment.billing_type === "self_payer"
													? "Selbstzahler"
													: appointment.billing_type === "private_patient"
													? "Privatpatient"
													: appointment.billing_type === "foreign_patient"
													? "Ausländischer Patient"
													: "Arbeitsunfall (BG)"}
											</p>
										</div>
									</div>
                    ...
                    )}>
                      {
                        ...
                        'Geplant'
								...
								<div className="mt-6 space-y-4">
									...
									{documents
										?.filter((doc) => doc.appointment_id === appointment.id)
										.map((document) => (
											<div
												key={document.id}
															...
															{editingDocumentId === document.id ? (
																<div className="space-y-2">
																	<textarea
																		...
																		onChange={(e) =>
																			setEditingDocumentComment(e.target.value)
																		}
																		rows={2}
																	...
																	<div className="flex justify-end space-x-2">
																		...
																		<button
																			onClick={() =>
																				handleUpdateDocumentComment(document.id)
																			}
								...
								<div className="mt-6">
									...
									{appointment.comments && appointment.comments.length > 0 ? (
										<div className="space-y-3 mb-4">
											{filterComments(appointment.comments).map(
												(comment: Comment) => (
													<div
														key={comment.id}
														className={cn(
													...
													>
														<div className="flex justify-between items-start">
															...
															{comment.is_todo && !comment.completed && (
																<button
																	onClick={() =>
																		handleMarkCommentAsCompleted(comment.id)
																	}
											...
											<div className="flex items-center space-x-4">
												<div className="flex items-center">
													<input
														...
														checked={
															getCommentForm(appointment.id).type === "info"
														}
														onChange={() =>
															updateCommentForm(appointment.id, {
																type: "info",
													...
													/>
													<label
														htmlFor={`info-type-${appointment.id}`}
												...
												<div className="flex items-center">
													<input
														...
														name={`comment-type-${appointment.id}`} // Eindeutiger Name pro Termin
														checked={
															getCommentForm(appointment.id).type === "anruf"
														}
														onChange={() =>
															updateCommentForm(appointment.id, {
																type: "anruf",
													...
													/>
													<label
														htmlFor={`anruf-type-${appointment.id}`}
														className="ml-2 text-sm text-gray-700 flex items-center"
													>
											...
											<div className="flex items-center space-x-4">
												<div className="flex items-center">
													<input
														...
														onChange={() => {
															const currentIsTodo = getCommentForm(
																appointment.id
															).isTodo;
															updateCommentForm(appointment.id, {
																isTodo: !currentIsTodo,
																// Reset priority when ToDo is unchecked
																priority: !currentIsTodo
																	? false
																	: getCommentForm(appointment.id).priority,
															});
														}}
														className="h-4 w-4 text-blue-600 focus:ring-blue-500"
													/>
													<label
														htmlFor={`todo-checkbox-${appointment.id}`}
												...
												{getCommentForm(appointment.id).isTodo && (
													<div className="flex items-center">
														<input
															...
															onChange={() =>
																updateCommentForm(appointment.id, {
																	priority: !getCommentForm(appointment.id)
																		.priority,
																})
															}
														...
														/>
														<label
															htmlFor={`priority-checkbox-${appointment.id}`}
															className="ml-2 text-sm text-gray-700 flex items-center"
														>
															<Flag
																className={`h-4 w-4 ${
