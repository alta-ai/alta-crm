	...
	const {
		...
		queryFn: async () => {
			try {
				...
				if (
					searchCriteria === "patient_number" &&
					isDateLike(debouncedSearchTerm)
				) {
					setWarning(
						"Die Eingabe sieht wie ein Datum aus. Möchten Sie stattdessen nach Geburtsdatum suchen?"
					);
					// Continue with search but handle potential errors gracefully
				} else if (
					searchCriteria === "birth_date" &&
					!isNaN(Number(debouncedSearchTerm)) &&
					!isDateLike(debouncedSearchTerm) &&
					debouncedSearchTerm.length > 2
				) {
					setWarning(
						"Die Eingabe sieht wie eine Zahl aus. Möchten Sie stattdessen nach Patientennummer suchen?"
					);
					// Continue with search but handle potential errors gracefully
				}
				...
				if (err.message?.includes("operator does not exist")) {
					setError(
						"Ein Fehler ist bei der Datumssuche aufgetreten. Bitte versuchen Sie ein anderes Format."
					);
				} else if (
					...
					setError(
						"Ein Fehler ist bei der Suche aufgetreten. Bitte versuchen Sie es erneut."
					);
				}
		...
		<div>
							...
							<input
								...
								placeholder={
									searchCriteria === "name"
										? "Nach Namen suchen..."
										: searchCriteria === "birth_date"
										? "Nach Geburtsdatum suchen (TT.MM.JJJJ)..."
			...
			{isSearching && patients && (
				<div className="mt-4 flex items-center text-sm text-gray-600">
					...
					<span className="ml-2">
						({patients.length}{" "}
						{patients.length === 1 ? "Ergebnis" : "Ergebnisse"})
					</span>
				</div>
			)}
