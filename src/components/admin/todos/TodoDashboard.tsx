...
import { 
  Check, Flag, Phone, Info, Search, Filter, AlertCircle, Calendar, User,
  X, Stethoscope
} from 'lucide-react';
...
const TodoDashboard = () => {
  ...
  const isFilterActive = typeFilter !== 'all' || statusFilter !== 'all' || priorityFilter !== 'all';
  ...
  if (statusFilter !== 'all') {
    todoTitle += ` • ${statusFilter === 'completed' ? 'Erledigt' : 'Unerledigt'}`;
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-semibold text-gray-900 mb-6">To-Do Übersicht</h1>
      ...
      <div className="bg-white shadow-sm rounded-lg overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-medium text-gray-900">
            {todoTitle} {filteredTodos.length > 0 && `(${filteredTodos.length})`}
          </h2>
        </div>
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <div className="flex items-center mb-1">
                      ...
                      {todo.priority && (
                        <Flag className="h-4 w-4 text-orange-500 ml-2" title="Priorität" />
                      )}
                      {todo.completed && (
                    ...
                    {todo.appointment && (
                      <div className="flex flex-wrap text-xs text-gray-500 space-x-3">
                        ...
                        <div className="flex items-center">
                          ...
                          {format(parseISO(todo.appointment.start_time), 'dd.MM.yyyy HH:mm', { locale: de })}
                          {todo.appointment.examination?.name && (
                            <>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      ...
                      {todo.user_profile?.first_name || "Unbekannt"} {todo.user_profile?.last_name || ""}
                      {' • '}
                      {format(parseISO(todo.created_at), 'dd.MM.yyyy HH:mm', { locale: de })}
                    </p>
                  </div>
