...
import { format, addDays, startOfWeek, endOfWeek, eachDayOfInterval, addWeeks, subWeeks, startOfMonth, endOfMonth, addMonths, subMonths, setMinutes, setHours, isSameDay, parseISO, isToday, isSameMonth, getMonth, addMinutes, isSaturday, isSunday } from 'date-fns';
import { de } from 'date-fns/locale';
import { ChevronLeft, ChevronRight, ZoomIn, ZoomOut, CalendarDays, Calendar, Grid3X3 } from 'lucide-react';
...
const CalendarView = ({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          ...
          previous_appointment:previous_appointment_id(start_time)
        `)
        .in('device_id', selectedDevices)
  ...
  const handleViewModeChange = (mode: 'day' | 'week' | 'workweek' | 'month') => {
    if (onViewModeChange) {
  ...
  const getAppointmentStyle = (appointment: Appointment, index: number, total: number) => {
    // Get status color from appointmentStatuses
    const status = appointmentStatuses?.find(s => s.id === appointment.status_id);
    const verschobenStatus = appointmentStatuses?.find(s => s.name === 'Verschoben');
  ...
  const DeviceHeader = ({ deviceId, index }: { deviceId: string, index: number }) => {
    const device = devices.find(d => d.id === deviceId);
    if (!device) return null;
    
    // Ermittle den Standortnamen, falls vorhanden
    const locationName = device.location_devices && device.location_devices.length > 0 
    ...
    return (
      <div className={`${colorClasses.bg} p-2 flex justify-between items-center border-b ${colorClasses.border} sticky top-0 z-30`}>
        ...
        >
          {selectedDevices.map((deviceId, deviceIndex) => {
            ...
            const dayAppointments = appointments?.filter(apt => 
              isSameDay(parseISO(apt.start_time), currentDate) && apt.device?.id === deviceId
            ) || [];
            ...
            return (
              <div key={deviceId} className="mb-6">
                ...
                <div className="grid" style={{ gridTemplateColumns: '60px 1fr', height: `${24 * zoomLevel}px` }}>
                      ...
                      <div key={hour} style={{ height: `${zoomLevel}px` }}>
                        {MINUTES.map(minute => {
                          ...
                          const gridSlot = timeGrid.find(
                            slot => slot.time.getHours() === hour && slot.time.getMinutes() === minute
                          );
                          
                          // Finde alle Termine dieses Geräts, die in diesem Slot beginnen
                          const startingAppointments = dayAppointments.filter(apt => {
                            const aptStart = parseISO(apt.start_time);
                            return (
                              aptStart.getHours() === hour && 
                              (Math.floor(aptStart.getMinutes() / 15) * 15) === minute
                            );
                          });
                          ...
                          return (
                            ...
                            >
                              ...
                              {startingAppointments.map((appointment, index) => {
                                const appointmentDuration = appointment.examination.duration;
                                const appointmentHeight = (appointmentDuration / 15) * (zoomLevel / 4);
                                
                                // Finde alle überlappenden Termine im gleichen Zeitfenster
                                const overlappingAppointments = gridSlot?.appointments || [];
                                ...
                                return (
                                  <button
                                    ...
                                    style={{
                                      ...
                                      minHeight: `${Math.max(zoomLevel / 4, 24)}px`,
                                      zIndex: 10
                                    }}
                                  >
                                    ...
                                    {zoomLevel >= 60 && appointment.patient && (
          ...
          return (
            <div key={deviceId} className="mb-6">
              ...
              <div className="overflow-hidden">
                <table className="w-full table-fixed border-collapse" style={{ height: `${24 * zoomLevel}px` }}>
                  <colgroup>
                    <col style={{ width: '60px' }} />
                    {days.map((_, i) => (
                      <col key={i} />
                    ...
                    {HOURS.map(hour => (
                      <tr key={hour} style={{ height: `${zoomLevel}px` }}>
                        ...
                        {days.map(day => {
                          ...
                          const dayAppointments = appointments?.filter(apt => 
                            isSameDay(parseISO(apt.start_time), day) && apt.device?.id === deviceId
                          ) || [];
                          
                          const grid = createTimeGrid(day, dayAppointments);
                          
                          return (
                            <td key={day.toString()} className="border-r border-gray-200 p-0 align-top">
              ...
              <div className="grid grid-cols-7 gap-px bg-gray-200">
                {days.map(day => {
                  ...
                  dayAppointments.forEach(apt => {
                    const timeKey = format(parseISO(apt.start_time), 'HH:mm');
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-1 space-y-1">
                        {Object.entries(timeGroups).map(([timeKey, appts], groupIndex) => {
                          return appts.map((appointment, index) => {
                            const style = getAppointmentStyle(appointment, index, appts.length);
    ...
    <div className="inline-flex rounded-md shadow-sm">
      <button
        ...
        className={cn(
          ...
          viewMode === 'day' ? "bg-blue-100 text-blue-700 z-10" : "bg-white text-gray-700 hover:bg-gray-50"
        )}
      >
      ...
      <button
        ...
        className={cn(
          ...
          viewMode === 'workweek' ? "bg-blue-100 text-blue-700 z-10" : "bg-white text-gray-700 hover:bg-gray-50"
        )}
      >
      ...
      <button
        ...
        className={cn(
          "relative inline-flex items-center px-3 py-2 text-sm font-medium border border-gray-300 -ml-px",
          viewMode === 'week' ? "bg-blue-100 text-blue-700 z-10" : "bg-white text-gray-700 hover:bg-gray-50"
        )}
      >
        <CalendarDays className="h-4 w-4 mr-1" />
        <span>Woche</span>
      ...
      <button
        ...
        className={cn(
          ...
          viewMode === 'month' ? "bg-blue-100 text-blue-700 z-10" : "bg-white text-gray-700 hover:bg-gray-50"
        )}
      >
