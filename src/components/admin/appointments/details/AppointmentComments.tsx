...
const AppointmentComments: React.FC<AppointmentCommentsProps> = ({
	appointmentId,
}) => {
	...
	const filterComments = (commentsToFilter: Comment[]) => {
		switch (commentFilter) {
			case "completed":
				return commentsToFilter.filter(
			...
			case "info":
				return commentsToFilter.filter(
					(comment) => comment.comment_type === "info"
				);
			case "anruf":
				return commentsToFilter.filter(
					(comment) => comment.comment_type === "anruf"
				);
			case "priority":
							...
							<div className="flex justify-between items-start">
								<div className="flex-1">
									...
									<p className="text-xs text-gray-500 mt-1">
										...
										{comment.user_profile?.first_name || "Unbekannt"}{" "}
						...
						{isTodo && (
							<div className="flex items-center">
								...
								>
									<Flag
										className={`h-4 w-4 ${
											priority ? "text-orange-500" : "text-gray-400"
										} mr-1`}
									/>
									Priorität
								</label>
