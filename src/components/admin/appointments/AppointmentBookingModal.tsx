...
type BillingType = 'self_payer' | 'private_patient' | 'foreign_patient' | 'work_accident';
...
const AppointmentBookingModal = ({
  ...
  const [step, setStep] = useState<'examination' | 'patient' | 'confirm'>('examination');
  ...
  useEffect(() => {
    if (isRescheduling && appointmentToReschedule) {
      ...
      setWithContrastMedium(appointmentToReschedule.patient_data?.with_contrast_medium || false);
      setBillingType(appointmentToReschedule.billing_type as BillingType);
      setInsuranceProviderId(appointmentToReschedule.insurance_provider_id || '');
      setHasBeihilfe(appointmentToReschedule.patient_data?.has_beihilfe || null);
      setHasTransfer(appointmentToReschedule.patient_data?.has_transfer || null);
  ...
  useEffect(() => {
    if (referringDoctorId) {
      const fetchDoctorDetails = async () => {
        try {
          const { data, error } = await supabase
            ...
            .select('id, title, first_name, last_name, specialty:medical_specialties(name), street, house_number, postal_code, city')
      ...
      if (!doctorSearchResults.find(d => d.id === referringDoctorId)) {
        ...
        setSelectedDoctor(doctorSearchResults.find(d => d.id === referringDoctorId));
      }
    } else {
  ...
  const { data: examinations } = useQuery({
    queryKey: ['examinations', deviceId, isRescheduling, appointmentToReschedule?.examination?.id],
    queryFn: async () => {
      let query = supabase
        .from('examinations')
        .select(`
  ...
  const getPrice = () => {
    if (!selectedExaminationData || billingType === 'work_accident') return null;

    const priceKey = `${billingType}_${withContrastMedium ? 'with' : 'without'}_contrast` as keyof ExaminationWithPrices;
  ...
  const handlePatientComplete = async (patientId: string) => {
    try {
      ...
      if (isRescheduling) {
        ...
        const { error: updateError } = await supabase
          ...
          .update({
            ...
            insurance_provider_id: billingType === 'private_patient' ? insuranceProviderId : null,
            patient_data: {
      ...
      setError(err.message || 'Der Termin konnte nicht gespeichert werden. Bitte versuchen Sie es später erneut.');
    }
  };
        ...
        <div className="p-4">
          ...
          <div className="mb-6 bg-gray-50 rounded-lg p-4 space-y-3">
            <div className="flex items-center text-sm text-gray-600">
              ...
              {format(selectedSlot, "EEEE, d. MMMM yyyy 'um' HH:mm 'Uhr'", { locale: de })}
            </div>
            {location && (
              <div className="flex items-center text-sm text-gray-600">
            ...
            {selectedExaminationData && (
              <div className="text-sm text-gray-600 pl-7">
                {selectedExaminationData.name} ({selectedExaminationData.duration} Minuten)
              </div>
            )}
            {selectedDoctor && (
              <div className="text-sm text-gray-600 pl-7">
                Überweiser: {selectedDoctor.title} {selectedDoctor.first_name} {selectedDoctor.last_name}
              </div>
            )}
          </div>
          ...
          {isRescheduling ? (
            <div className="space-y-6">
              <div className="bg-white rounded-lg border border-gray-200 p-4">
                <h3 className="text-sm font-medium text-gray-900 mb-4">Termin bestätigen</h3>
                <div className="space-y-2">
                  <p className="text-sm text-gray-600">
                    <span className="font-medium">Patient:</span> {appointmentToReschedule.patient.first_name} {appointmentToReschedule.patient.last_name}
                  </p>
                  <p className="text-sm text-gray-600">
                    <span className="font-medium">Untersuchung:</span> {appointmentToReschedule.examination.name}
                  </p>
                  <p className="text-sm text-gray-600">
                    <span className="font-medium">Abrechnungsart:</span> {BILLING_TYPE_LABELS[appointmentToReschedule.billing_type]}
                  </p>
                  {selectedDoctor && (
                    <p className="text-sm text-gray-600">
                      <span className="font-medium">Überweiser:</span> {selectedDoctor.title} {selectedDoctor.first_name} {selectedDoctor.last_name}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="flex justify-end space-x-3">
                ...
                <button
                  onClick={() => handlePatientComplete(appointmentToReschedule.patient.id)}
                      ...
                      <div className="mt-1">
                        {referringDoctorId ? (
                          ...
                          <div>
                            ...
                            {/* Suchergebnisse */}
                            {isSearchingDoctors ? (
                              <div className="mt-1 p-2 text-sm text-gray-500">Suche läuft...</div>
                              ...
                              <div className="mt-1 p-2 text-sm text-gray-500">
                                ...
                                <Link to="/admin/referring-doctors/new" className="ml-1 text-blue-600 hover:text-blue-800">
                      ...
                      <div className="space-y-2">
                        <label className="flex items-center">
                          <input
                            ...
                            className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300"
                          />
                          <span className="ml-2 text-sm text-gray-700">Ohne Kontrastmittel</span>
                        </label>
                        <label className="flex items-center">
                          <input
                            type="radio"
                            ...
                            className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300"
                          />
                          <span className="ml-2 text-sm text-gray-700">Mit Kontrastmittel</span>
                        </label>
                      </div>
                    </div>
                  )}
                      ...
                      <div className="flex items-start">
                        ...
                        <div>
                          <p className="text-sm font-medium text-gray-900">
                            Preis: {new Intl.NumberFormat('de-DE', { style: 'currency', currency: 'EUR' }).format(getPrice() || 0)}
                          </p>
                      ...
                      <div className="flex items-start">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-yellow-700 mt-1">
                            Die Abrechnung erfolgt direkt mit der zuständigen Berufsgenossenschaft.
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex justify-end">
                    <button
                      ...
                      disabled={
                        ...
                        (billingType === 'private_patient' && (!insuranceProviderId || hasBeihilfe === null))
                      }
